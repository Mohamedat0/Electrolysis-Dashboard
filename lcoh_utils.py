...
def calculate_lcoh(tech_type, region, stack_data, bop_epc_data, wacc, fom, utilization_rate,
                   electricity_cost, electrolyzer_efficiency, learning_model='second_layer', bop_epc_model='local', year_index=None):
    """
    ...
    capital_component = (annual_capital_costs + fom) / (
        8760 * safe_utilization_rate)  # $ per kWh of capacity
    electricity_component = electricity_cost  # $ per kWh

    # Calculate LCOH components - overwriting previous calculation for clarity
    capital_component = (crf * capex) / (8760 * safe_utilization_rate)
    fom_component = fom / (8760 * safe_utilization_rate)
