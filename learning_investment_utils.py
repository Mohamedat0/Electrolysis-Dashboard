...
def generate_bop_epc_learning_investments(
    ...
    for region in regions:
        ...
        learning_investments['local'][region] = calculate_bop_epc_learning_investment_local(
            region, costs_0, {}, alphas, bop_epc_data
        )
        
        # Global learning model
        learning_investments['global'][region] = calculate_bop_epc_learning_investment_global(
            region, costs_0, regions, alphas, bop_epc_data
        )
        
        # Convert to DataFrames
        for model in learning_investments:
            learning_investments[model][region] = pd.DataFrame(learning_investments[model][region])
