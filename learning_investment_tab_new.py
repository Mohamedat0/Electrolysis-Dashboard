...
def implement_stack_investment_tab(TECHNOLOGIES, get_stack_display_name, 
    ...
    **Note on capacity calculations:** 
    The learning investment calculations include additional capacity beyond the base data:
    - 1.1 GW added to PEM technologies
    - 22.58 GW added to ALK technologies
    
    These capacities are added according to the learning model:
    - Shared learning: All additional capacity (both PEM and ALK) is added to the total
    - Technological Fragmentation: PEM additional capacity is added to PEM technologies, ALK to ALK technologies
    - Regional Fragmentation: The relevant additional capacity is added to the specific technology
    """)
    ...
    for model in learning_models:
        ...
        fig_inv.add_trace(
            go.Scatter(
                ...
                y=model_data['learning_investment'] / 1e6,  # Convert to millions
                mode='lines',
    ...
    for model in learning_models:
        ...
        fig_cap.add_trace(
            go.Scatter(
                x=model_data['target_cost'],
                y=model_data['required_capacity'] / 1000,  # Convert from MW to GW
                mode='lines',
                name=model_display_names[model],
                line=dict(color=model_colors[model], width=3),
    ...
    for model in learning_models:
        ...
        # Get the closest row to target cost
        ...
        comparison_data.append({
            ...
            'Required Capacity (GW)': target_row['required_capacity'] / 1000,  # Convert MW to GW
            'Learning Investment ($ billion)': target_row['learning_investment'] / 1e6
        })
    ...
    comparison_df['Required Capacity (GW)'] = comparison_df['Required Capacity (GW)'].round(1)
    comparison_df['Learning Investment ($ billion)'] = comparison_df['Learning Investment ($ billion)'].round(2)
...
def implement_bop_epc_investment_tab(get_region_display_name, bop_epc_costs_0, 
    ...
    bop_comparison_df['Required Capacity (GW)'] = bop_comparison_df['Required Capacity (GW)'].round(1)
    bop_comparison_df['Learning Investment ($ billion)'] = bop_comparison_df['Learning Investment ($ billion)'].round(2)
