...
def generate_regional_bop_epc_data(
    ...
    """
    # Define the additional capacities to add - same as in generate_regional_stack_data
    additional_pem_capacity = 1100  # 1.1 GW in MW
    additional_alk_capacity = 22580  # 22.58 GW in MW
    ...
    # Calculate total initial capacity across all regions
    base_x_0_total = sum([regional_capacities[region]['capacity'][0] for region in regions])
    
    # Add the additional capacities for PEM and ALK to the total initial capacity
    x_0_total = base_x_0_total + additional_pem_capacity + additional_alk_capacity
    ...
    for region in regions:
        ...
        results['local'][f"{region}_pem"] = {
            'year': year_range,
            'capacity': [0] * (years + 1),  # Initialize with zeros
            'cost': [costs_0_pem[region]] * (years + 1)  # Initialize with initial cost
        }
    ...
    for region in regions:
        ...
        for year_idx in range(0, years + 1):
            if year_idx == 0:
                ...
                # For consistent learning curve calculations, add additional capacities to both the numerator and denominator
                # Add additional PEM capacity (1.1 GW)
                adjusted_capacity_pem = region_capacity_pem + additional_pem_capacity
                adjusted_initial_capacity_pem = region_initial_capacity_pem + additional_pem_capacity
                
                # Add additional ALK capacity (22.58 GW)
                adjusted_capacity_alk = region_capacity_alk + additional_alk_capacity  
                adjusted_initial_capacity_alk = region_initial_capacity_alk + additional_alk_capacity

                # Calculate total regional capacity (PEM + ALK) including additional capacities
                region_total_capacity = adjusted_capacity_pem + adjusted_capacity_alk
                region_total_initial_capacity = adjusted_initial_capacity_pem + adjusted_initial_capacity_alk
                ...
                for r in regions:
                    ...
                    raw_global_capacities.append(r_capacity_pem + r_capacity_alk)
                
                # Raw total without additional capacities
                raw_total_global_capacity = sum(raw_global_capacities)
                
                # Add the additional capacities consistently to global total
                total_global_capacity = raw_total_global_capacity + additional_pem_capacity + additional_alk_capacity

                # Add the additional capacities to the initial total as well for the denominator (should match initialization)
                adjusted_x_0_total = x_0_total  # This should already include additional capacities

                # Set minimum total capacity to 100MW
                adjusted_x_0_total = max(100, adjusted_x_0_total)

                # Store global costs using region-specific learning rate
                global_learning_factor = (total_global_capacity / adjusted_x_0_total) ** alphas_pem[region]
