...
# Initialize default values for all parameters if they don't exist
def initialize_defaults():
    ...
    for region in ["usa", "eu", "china", "row"]:
        ...
        defaults.update(region_costs)
    
    # Capacities - 100 MW for each technology in each region
    for region in ["usa", "eu", "china", "row"]:
        for tech in ["wpem", "walk", "cpem", "calk"]:
            defaults[f"{region}_{tech}_cap"] = 100.0
    
    # Growth rates - 10% for each technology in each region
    for region in ["usa", "eu", "china", "row"]:
        for tech in ["wpem", "walk", "cpem", "calk"]:
            defaults[f"{region}_{tech}_growth"] = 10.0
    
    # Apply defaults to session state for any missing values
...
st.title("Electrolysis Cost Projections")
st.markdown("""
...
with st.sidebar:
    ...
    if st.button("Reset to Defaults", use_container_width=True, type="primary"):
        # Reset to default values by setting the session state variables directly
        
        # Reset stack parameters
        st.session_state.wpem_lr = 20.0
        st.session_state.walk_lr = 20.0
        st.session_state.cpem_lr = 20.0
        st.session_state.calk_lr = 20.0
        
        # Reset stack costs
        st.session_state.wpem_cost = 600.0
        st.session_state.walk_cost = 340.0
        st.session_state.cpem_cost = 600.0
        st.session_state.calk_cost = 110.0
        
        # Reset BoP & EPC learning rates
        st.session_state.usa_lr = 12.5
        st.session_state.eu_lr = 12.5
        st.session_state.china_lr = 12.5
        st.session_state.row_lr = 12.5
        
        # Reset BoP & EPC costs (PEM)
        st.session_state.usa_cost_pem = 1900.0
        st.session_state.eu_cost_pem = 1900.0
        st.session_state.china_cost_pem = 430.0
        st.session_state.row_cost_pem = 1160.0
        
        # Reset BoP & EPC costs (ALK)
        st.session_state.usa_cost_alk = 2150.0
        st.session_state.eu_cost_alk = 2150.0
        st.session_state.china_cost_alk = 490.0
        st.session_state.row_cost_alk = 1320.0
        
        # Reset capacity parameters - 100 MW for each technology in each region
        for region in ["usa", "eu", "china", "row"]:
            for tech in ["wpem", "walk", "cpem", "calk"]:
                st.session_state[f"{region}_{tech}_cap"] = 100.0
        
        # Reset growth rate parameters - 10% for each technology in each region
        for region in ["usa", "eu", "china", "row"]:
            for tech in ["wpem", "walk", "cpem", "calk"]:
                st.session_state[f"{region}_{tech}_growth"] = 10.0
        
        # Reset years and base year
        if "years" in st.session_state:
            st.session_state.years = 20
        if "base_year" in st.session_state:
            st.session_state.base_year = 2023
        
        # Force the app to rerun with updated values
        st.rerun()
            ...
            with growth_tabs[region_idx]:
                ...
                with col1:
                    tech_growth_rates['western_pem'] = st.slider(
                        ...
                        min_value=0.0,
                        max_value=50.0,
                        value=10.0,  # Default value of 10%
                        step=1.0,
...
# Add a note about the capacity adjustments
st.info("""
**Note on capacity calculations:** 
Additional capacity has been added to the initial capacity values in the stack projection calculations:
- 1.1 GW added to PEM technologies (Western and Chinese)
- 23.68 GW added to ALK technologies (Western and Chinese)
""")
